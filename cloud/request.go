...
func request(provider *ProviderConfig, method string, path string, body interface{}) (interface{}, error) {
	...
	if err != nil {
		if res.StatusCode != 200 {
			return nil, fmt.Errorf("bad status code from IMS Cloud API: %d", res.StatusCode)
		} else {
			return nil, fmt.Errorf("invalid response from IMS Cloud API: %+v", result)
		}
	}
	if res.StatusCode != 200 {
		return nil, fmt.Errorf("error response from IMS Cloud API (%d): %+v", res.StatusCode, result)
	}
	return result, nil
}
...
func waitCommand(provider *ProviderConfig, commandID string) (map[string]interface{}, error) {
	...
	for {
		if startTime.Add(40*time.Minute).Sub(time.Now()) < 0 {
			return nil, errors.New("timeout waiting for IMS Cloud command to complete")
		}
		...
		if len(commands) != 1 {
			return nil, errors.New("invalid response from IMS Cloud queue API: invalid number of command responses")
		}
		...
		if hasStatus {
			...
			case "error":
				...
				if hasLog {
					return nil, fmt.Errorf("cloud command failed: %s", log)
				} else {
					return nil, fmt.Errorf("cloud command failed: %v", command)
				}
			}
		}
	}
